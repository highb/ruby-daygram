...
RSpec.describe Daygram::CLI do
  let(:options) { [] }
  let(:command_line) { Array(command).concat options }
  let :cli do
    lambda do
      load "daygram/cli.rb" # Ensures clean Thor `.method_option` evaluation per spec.
      described_class.start command_line
    end
  end

  shared_examples_for "a config command", :temp_dir do
    let(:configuration_path) { File.join temp_dir, Daygram::Identity.file_name }
  ...
  shared_examples_for "a version command" do
    it "prints version" do
      expect(&cli).to output(/Daygram\s#{Daygram::Identity.version}\n/).to_stdout
    end
  end

  shared_examples_for "a help command" do
    it "prints usage" do
      expect(&cli).to output(/Daygram\s#{Daygram::Identity.version}\scommands:\n/).to_stdout
    end
  end
